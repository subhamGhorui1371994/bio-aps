...
class StateListController extends Controller
{
   /**
     ...
     * @return false|string
     */
    public function state_list_list_ajax(Request $request)
    {
        $limit = $offset = 0;
        ...
        if (isset($request->order[0])) {
            if (isset($request->order[0]['column'])) {
                if ($request->order[0]['column'] == 0) {
                    $order_column = 'state_id';
                }
                elseif ($request->order[0]['column'] == 1) {
                    $order_column = 'state_name';
                }
                elseif ($request->order[0]['column'] == 2) {
                    $order_column = 'state_code';
                }
                elseif ($request->order[0]['column'] == 3) {
                    $order_column = 'union_territory';
                }
            }
        }
        ...
        $details = StateList::getListDataTable($order_column, $order_column_by, $limit, $offset, $search);
